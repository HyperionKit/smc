...
async function main() {
  ...
  for (const config of tokenConfigs) {
    ...
    // Get token contract
    const token = await ethers.getContractAt("SimpleERC20", config.address);
    ...
    // Test deposit to Lazchain
    const destinationChainId = NETWORKS.LAZCHAIN;
    const destinationAddress = user.address; // Same address on destination chain

    console.log(`   Depositing ${ethers.formatUnits(config.amount, config.decimals)} ${config.symbol} to Lazchain (Chain ID: ${destinationChainId})...`);
