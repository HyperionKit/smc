...
describe("DeFi System", function () {
  ...
  beforeEach(async function () {
    ...
    // Deploy tokens
    const SimpleERC20 = await ethers.getContractFactory("contracts/SimpleERC20.sol:SimpleERC20");
    
    usdt = await SimpleERC20.deploy("Tether USD", "USDT", 6, ethers.parseUnits("40000000", 6));
    usdc = await SimpleERC20.deploy("USD Coin", "USDC", 6, ethers.parseUnits("40000000", 6));
    dai = await SimpleERC20.deploy("Dai Stablecoin", "DAI", 18, ethers.parseUnits("40000000", 18));
    weth = await SimpleERC20.deploy("Wrapped Ether", "WETH", 18, ethers.parseUnits("40000000", 18));
