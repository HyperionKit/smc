...
async function swapWETHToUSDT() {
  console.log("🔄 SMC DeFi Swap: WETH → USDT");
  console.log("=================================");
  ...
  try {
    ...
    const usdt = await ethers.getContractAt("Token", usdtAddress);
    ...
    // Get pool info
    const pool = await amm.getPool(wethAddress, usdtAddress);
    ...
    console.log("Pool reserves - WETH:", ethers.formatUnits(pool.reserveA, 18), "USDT:", ethers.formatUnits(pool.reserveB, 6));
    ...
    const swapTx = await amm.connect(user).swap(wethAddress, usdtAddress, swapAmount, 0); // minAmountOut = 0 for testing
    await swapTx.wait();
    ...
    console.log("\n🎉 WETH → USDT swap completed successfully!");
