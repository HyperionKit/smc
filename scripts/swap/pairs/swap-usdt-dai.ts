...
async function main() {
  console.log("=== USDT to DAI Swap Script ===\n");
  ...
  const daiContract = await ethers.getContractAt("Token", DAI_ADDRESS, signer);
  ...
  try {
    ...
    // Get pool info
    const poolInfo = await ammContract.getPool(USDT_ADDRESS, DAI_ADDRESS);
    ...
    console.log(`Effective Rate: 1 USDT = ${ethers.formatUnits(daiReceived * ethers.parseUnits("1", 6) / usdtSpent, 18)} DAI`);

    // Calculate fees
    const fee = usdtSpent - daiReceived;
