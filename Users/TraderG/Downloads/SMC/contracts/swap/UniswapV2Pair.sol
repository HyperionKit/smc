...
contract UniswapV2Pair is ERC20, Ownable {
    ...
    constructor() ERC20("Uniswap V2", "UNI-V2") Ownable(msg.sender) {}
    ...
    function _update(uint balance0, uint balance1, uint112 _reserve0, uint112 _reserve1) private {
        ...
        if (timeElapsed > 0 && _reserve0 != 0 && _reserve1 != 0) {
            // Simplified price calculation without UQ112x112
    ...
    function mint(address to) external lock returns (uint liquidity) {
        ...
        if (_totalSupply == 0) {
            liquidity = UniswapV2Library.sqrt(amount0 * amount1) - MINIMUM_LIQUIDITY;
    ...
    function _mintFee(uint112 _reserve0, uint112 _reserve1) private returns (bool feeOn) {
        ...
        if (feeOn) {
            if (_kLast != 0) {
                uint rootK = UniswapV2Library.sqrt(uint(_reserve0) * uint(_reserve1));
                uint rootKLast = UniswapV2Library.sqrt(_kLast);
                if (rootK > rootKLast) {
